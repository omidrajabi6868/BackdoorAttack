...
def main(model_filepath, result_filepath, scratch_dirpath, examples_dirpath, example_img_format='png'):
    ...
    from utils import TheDataset
    import Network as N

    dataset_cl = TheDataset('cifar10', mode='test', poison_transform=None, normalizing=True)
    test_loader_cl = torch.utils.data.DataLoader(dataset_cl, batch_size=50, shuffle=True)
    fxs, fys = next(iter(test_loader_cl))
    fxs, fys = fxs.numpy(), fys.numpy()

    image_mins = []
    image_maxs = []
    for image in fxs:
        image_mins.append(np.min(image))
        image_maxs.append(np.max(image))
    ...
    model = load_model(model_filepath, 'resnet18', 10)
    ...
    if model_type  == 'ResNet':
        ...
        target_layers = ['Sequential', 'AdaptiveAvgPool2d']
    elif model_type == 'Inception3':
        children = list(model.children())
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_filepath', type=str, help='File path to the pytorch model file to be evaluated.', default='./models/model_cifar10_resnet18_10_clean.pth')
    parser.add_argument('--result_filepath', type=str, help='File path to the file where output result should be written. After execution this file should contain a single line with a single floating point trojan probability.', default='./abs_output/output')
    parser.add_argument('--scratch_dirpath', type=str, help='File path to the folder where scratch disk space exists. This folder will be empty at execution start and will be deleted at completion of execution.', default='./abs_output/scratch')
    # parser.add_argument('--examples_dirpath', type=str, help='File path to the folder of examples which might be useful for determining whether a model is poisoned.', default='./abs_output/example')

    args = parser.parse_args()

    main(args.model_filepath, args.result_filepath, args.scratch_dirpath, None)